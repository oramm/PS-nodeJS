...
export default class ToolsDocs {
     ...
     */
    static async initNamedRangesFromTags(auth: OAuth2Client, documentId: string) {
        let document = await (await this.getDocument(auth, documentId)).data;
        const tags = this.getTagsForNamedRanges(document);
        if (!tags.length)
            throw new Error('No tags for namedRanges found');

        await this.clearNamedRanges(auth, documentId);
        const namedRangesCreated = await this.refreshNamedRangesFromTags(auth, documentId);
     ...
     * https://developers.google.com/docs/api/samples/output-json?hl=en
     */
    static async refreshNamedRangesFromTags(auth: OAuth2Client, documentId: string) {
        ...
        paragraphs.forEach(element => {
            if (element.paragraph) {
                const request = this.createNameRangeRequestFromTextRun(element.paragraph)
                const name = request?.createNamedRange?.name;
                let namedRange;
                if (request && name) {
                    console.log(`RangeName:: ${name}, s-e: ${request.createNamedRange?.range?.startIndex}-${request.createNamedRange?.range?.endIndex}`)
    ...
    private static createNameRangeRequestFromTextRun(paragraph: docs_v1.Schema$Paragraph): docs_v1.Schema$Request | undefined {
            ...
            for (const textElement of textElements) {
                ...
                if (textRunContent && namedRangeRegex.test(textRunContent)) {
                    if (textElement.startIndex) {
                        const namedRangeName = this.templateTagToRangeName(textRunContent);
                        return {
                            createNamedRange: {
                                name: namedRangeName,
                                range: {
    ...
    static async updateTextRunsInNamedRanges(auth: OAuth2Client, documentId: string, newData: { rangeName: string, newText: string }[]) {
        const notMatchedData = [...newData];
        const document = (await this.getDocument(auth, documentId)).data;

        if (document.namedRanges) {
            //this.sortNamedRangesDescending() zwraca nową tablicę posortowaną
            const namedRangesSorted = this.sortNamedRangesDescending(document.namedRanges);
            ...
            for (const sharingNameNamedRanges of namedRangesSorted) {
                for (const dataElement of newData) {
                    ...
                    if (sharingNameNamedRanges.name == dataElement.rangeName && sharingNameNamedRanges.namedRanges) {
                        const index = notMatchedData.findIndex(obj => obj.rangeName === dataElement.rangeName)
                        notMatchedData.splice(index, 1);
                        console.log(`updateTextRunInNamedRange(${sharingNameNamedRanges.namedRanges[0].name},${dataElement.newText})`)
                        console.group();
                        await this.updateTextRunInNamedRange(auth, document, sharingNameNamedRanges.namedRanges[0], dataElement.newText);
                        console.groupEnd();
                        console.log(`updateTextRunInNamedRange(${sharingNameNamedRanges.namedRanges[0].name},${dataElement.newText}) DONE`);
                        continue;
                    }
                }
            }
            if (notMatchedData.length > 0)
                console.log('Some tags did not match any of namedRanges %o:', notMatchedData);
        }
    }
    ...
    static async updateTextRunInNamedRange(
        ...
        if (!document.body?.content)
            throw new Error(`Document ${document.title} has no content: ${namedRange.name}`)
        if (!namedRange.ranges || !namedRange.ranges[0].startIndex || !namedRange.ranges[0].endIndex)
            throw new Error(`NamedRange ${namedRange.name} is not attached to document`);
        const startIndex = namedRange.ranges[0].startIndex;
        const endIndex = namedRange.ranges[0].endIndex;
        const allParagraphs = this.getAllParagraphElementsFromDocument(document.body.content);
        const textRunAndParenElement = this.getTextRunsAndParagraphElement(allParagraphs, startIndex, endIndex);
        const textRunElements = textRunAndParenElement?.textRunElements;
        if (!textRunElements) {
            console.log('textRunAndParenElement?.parentElement %o', textRunAndParenElement?.parentElements);
            console.log('namedRange %o', namedRange);
            throw new Error(`No textRun found for namedRange: ${namedRange.name}`);

        }

        const requestIndexes = this.setUpdateNamedRangeIndexes(textRunAndParenElement.parentElements, textRunElements, startIndex, endIndex)

        this.LogInConsoleParagraphsAndTextruns(requestIndexes, textRunAndParenElement);
        ...
        try {
            ...
            await this.batchUpdateDocument(auth, requests, <string>document.documentId);
        } catch (error) {
    ...
    private static LogInConsoleParagraphsAndTextruns(requestIndexes: {
        ...
        for (let i = 0; i < textRunAndParenElement.parentElements.length; i++) {
            const paragraph = textRunAndParenElement.parentElements[i].paragraph;
            if (paragraph) {
                paragraphs[`paragraph${i}`] = {
                    startIndex: textRunAndParenElement.parentElements[i].startIndex,
     ...
     */
    private static setUpdateNamedRangeIndexes(parentParagraphs: docs_v1.Schema$StructuralElement[], affectedTextRunElements: docs_v1.Schema$ParagraphElement[], namedRangeStartIndex: number, namedRangeEndIndex: number) {
        const indexes = {
        ...
        parentParagraphs.forEach(paragraphElement => {
            if (paragraphElement?.paragraph?.elements)
                allParentParagraphsTextRuns = allParentParagraphsTextRuns.concat(paragraphElement?.paragraph?.elements)
        });

        if (!allParentParagraphsTextRuns) throw new Error('Paragraph is empty');
        allParentParagraphsTextRuns = allParentParagraphsTextRuns.filter(element => element.textRun);

        const paragraphMergedContent: string = allParentParagraphsTextRuns.map(element => {
        ...
        const startRelative = namedRangeStartIndex - <number>firstParagraphstartIndex;
        const endRelative = namedRangeEndIndex - <number>firstParagraphstartIndex;
        const paragraphOutsideMergedCOntent = Tools.getRemainingString(startRelative, endRelative, paragraphMergedContent);

        const lastCharInParagraph = paragraphOutsideMergedCOntent[paragraphOutsideMergedCOntent.length - 1];
    ...
    private static makeTextRunUpdateRequests(
        ...
        requests.push({
            createNamedRange: {
                ...
                range: {
                    startIndex: requestIndexes.namedRangeStartIndex,
                    endIndex: requestIndexes.namedRangeStartIndex + newText.length
                }
            }
        });
        ...
        if (style) {
            requests.push({
                updateTextStyle: {
                    range: {
                        startIndex: requestIndexes.namedRangeStartIndex,
                        endIndex: <number>requestIndexes.namedRangeStartIndex + newText.length
                    },
                    textStyle: style,
                    fields: '*'
                }
    ...
    */
    ...
    static getTagsForNamedRanges(document: docs_v1.Schema$Document) {
        if (!document.body || !document.body.content)
            throw new Error('No template document found or template empty');
        const content = document.body.content;
        const paragraphElements = this.getAllParagraphElementsFromDocument(content);
    ...
    /**@deprecated */
    private static getlastTwoTextRunsFromTableCell(cell: docs_v1.Schema$TableCell) {
        ...
        if (cell.content && cell.content?.length >= 2) {
            for (let i = cell.content.length - 1; i === 0; i--) {
                ...
                if (lastParagraph?.elements)
                    lastTextRun = lastParagraph.elements[lastParagraph.elements.length].textRun;
                if (i) {
                    penultimateParagraph = cell.content[i - 1].paragraph;
                    if (penultimateParagraph?.elements)
                        penultimateTextRun = penultimateParagraph.elements[penultimateParagraph.elements.length].textRun;
                }
            }
        }
        return { penultimateParagraph, lastParagraph, penultimateTextRun, lastTextRun };
    }

    private static getParagraphParentTableCell(obj: docs_v1.Schema$StructuralElement[] | docs_v1.Schema$StructuralElement | any, endIndex: number) {
        //console.group('-------------isParagraphInTableCell------------')
        if (Array.isArray(obj)) {
            for (const item of obj) {
                if (this.getParagraphParentTableCell(item, endIndex))
                    return;
            }
            ...
            for (const cell of obj.tableCells) {
                ...
                if (cell.endIndex === endIndex) {
                    //console.log('MATCH!!!')
                    const lastElements = this.getlastTwoTextRunsFromTableCell(cell);
                    return {
    ...
    private static templateTagToRangeName(tag: string): string {
        return tag.replace(/#ENVI#/, '').replace(/#/, '').trim();
    }
     ...
     */
    static getAllParagraphElementsFromDocument(obj: docs_v1.Schema$StructuralElement[] | docs_v1.Schema$StructuralElement | any): docs_v1.Schema$StructuralElement[] {
        let paragraphElements: docs_v1.Schema$StructuralElement[] = [];

        if (Array.isArray(obj)) {
            for (const item of obj)
                paragraphElements = paragraphElements.concat(this.getAllParagraphElementsFromDocument(item));
        } else if (typeof obj === 'object' && obj !== null) {
            if (obj.paragraph) {
                paragraphElements.push(obj);
            } else {
                for (const key in obj)
                    paragraphElements = paragraphElements.concat(this.getAllParagraphElementsFromDocument(obj[key]));
            }
        }
        return paragraphElements;
    }

    static paragraphContainsText(paragraph: docs_v1.Schema$Paragraph, textRunContentRegexp: RegExp | string) {
     ...
     */
    private static getTextRunsAndParagraphElement(paragraphElements: docs_v1.Schema$StructuralElement[], startIndex: number, endIndex: number) {
        ...
        for (const element of paragraphElements) {
            if (!element.paragraph?.elements || element.paragraph?.elements?.length === 0) {
                throw new Error('Function getTextRunElementByIndexes cannot be used for empty Pragraphs');
            }
            ...
            if (element.startIndex < endIndex && element.endIndex > startIndex && endIndex > startIndex) {
                affectedParagraphs.push(element);
                //czy paragraph jst mniejszy niz zakres

                const foundElements = element.paragraph.elements.filter(element => {
                    if (!element.startIndex || !element.endIndex || !element.textRun) return false;
                    return element.endIndex > startIndex || element.startIndex < endIndex;
                });

                affectedTextRunElements.push(...foundElements);
            }
            const lastTextrunElement = affectedTextRunElements[affectedTextRunElements.length - 1];
            if (lastTextrunElement && lastTextrunElement.endIndex && lastTextrunElement.endIndex >= endIndex)
                return {
    ...
    private static getTextRunByStartIndex(paragraph: docs_v1.Schema$Paragraph, startIndex: number) {
    ...
    private static sortNamedRangesDescending(namedRanges: docs_v1.Schema$NamedRanges): docs_v1.Schema$NamedRanges[] {
        return Object.values(namedRanges).sort((a, b) => {
            return b.namedRanges[0].ranges[0].startIndex - a.namedRanges[0].ranges[0].startIndex;
        });
    }
