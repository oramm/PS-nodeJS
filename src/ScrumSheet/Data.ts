...
export default class Data {
    static async synchronizePersonsInScrum(
        ...
        const personNameColIndex = dataValues[0].indexOf(
            Setup.ScrumSheet.Data.personNameColName
        );

        await ToolsSheets.clearValues(auth, {
            spreadsheetId: Setup.ScrumSheet.GdId,
            range:
                `'${Setup.ScrumSheet.Data.name}'!R${
                    Setup.ScrumSheet.Data.firstDataRow
                }C${personIdColIndex + 1}:` +
                `R${dataValues.length}C${personNameColIndex + 1}`,
        });
        const personsData = persons.map((item) => [
            item.id,
    ...
    static async synchronizeCasesData(auth: OAuth2Client) {
        ...
        const caseGdFolderIdColIndex = dataValues[0].indexOf(
            Setup.ScrumSheet.Data.caseGdFolderIdColName
        );
