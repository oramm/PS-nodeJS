...
export default class Project extends BusinessObject implements ProjectData {
    ...
    async setProjectEntityAssociationsFromDb() {
        let sql =
            ...
            'WHERE Projects.OurId="' +
            this.ourId +
            '" \n' +
            'ORDER BY Projects_Entities.ProjectRole, Entities.Name';

        const result: any[] = <any[]>await ToolsDb.getQueryCallbackAsync(sql);
        const allEntitiesInProject =
            this.processProjectEntityAssociations(result);
     ...
     */
    async addNewProjectEmployersAssociationsInDb(
     ...
     * tworzy instancję procesu i zadanie do scrumboarda na podstawie szablonu
     */
    async addNewProjectEngineersAssociationsInDb(
        externalConn?: mysql.PoolConnection,
        isPartOfTransaction?: boolean
    ) {
        if (this._engineers) {
     ...
     */
    async editProjectEntitiesAssociationsInDb(
        externalConn?: mysql.PoolConnection,
        isPartOfTransaction?: boolean
    ) {
        await this.deleteProjectEntityAssociations(
            externalConn,
            isPartOfTransaction
        );
        await this.addNewProjectEmployersAssociationsInDb(
            externalConn,
            isPartOfTransaction
        );
        await this.addNewProjectEngineersAssociationsInDb(
            externalConn,
            isPartOfTransaction
        );
    }
    async deleteProjectEntityAssociations(
        ...
        return await ToolsDb.executePreparedStmt(
            sql,
            [ToolsDb.prepareValueToSql(this.id)],
            this
        );
    }
