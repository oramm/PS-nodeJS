...
export default class LetterGdController {
    ...
    static async createLetterFolder(auth: OAuth2Client, letterData: Letter) {
        if (!letterData._project.lettersGdFolderId) throw new EnviErrors.NoGdIdError('Project must have lettersGdFolderId');
     ...
     **/
    static async appendAttachments(auth: OAuth2Client, files: Express.Multer.File[], letterFolderGdId: string) {
        await this.uploadAttachments(auth, files, letterFolderGdId);
    }
     ...
     */
    protected static async uploadAttachments(auth: OAuth2Client, files: Express.Multer.File[], parentFolderGdId: string) {
        const promises = files.map((file) => {
            return ToolsGd.uploadFileMulter(auth, file, undefined, parentFolderGdId);
            //blobEnvi.parent = parentFolderGdId;
            //promises.push(ToolsGd.uploadFileBase64(auth, blobEnvi, undefined, parentFolderGdId));
        });
